	...
	*/
	...
	function saveFeed($filename="", $displayContents=true) {
		...
		if ($feedFile) {
			...
			echo "<br /><b>Erro ao criar arquivo para feed, por favor verifique as permissões.</b><br />";
		}
	}
...
class FeedDate {
	...
	function FeedDate($dateString="") {
		...
		if (preg_match("~(?:(?:Seg|Ter|Qua|Qui|Sex|Sab|Dom),\\s+)?(\\d{1,2})\\s+([a-zA-Z]{3})\\s+(\\d{4})\\s+(\\d{2}):(\\d{2}):(\\d{2})\\s+(.*)~",$dateString,$matches)) {
