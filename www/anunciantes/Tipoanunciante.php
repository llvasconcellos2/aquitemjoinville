...
function findAll() {
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `tipoanunciante` $where"); 
	...
	$query_recordset = "SELECT idTipo,tipo FROM `tipoanunciante` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `tipoanunciante` (tipo) VALUES (%s)" ,			GetSQLValueString($_REQUEST["tipo"], "text")# 
	);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `tipoanunciante` WHERE idTipo = %s",
		GetSQLValueString($_REQUEST["idTipo"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `tipoanunciante` SET tipo = %s WHERE idTipo = %s", 
			GetSQLValueString($_REQUEST["tipo"], "text"), 
			GetSQLValueString($row_recordset["idTipo"], "int")
		);
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `tipoanunciante` WHERE idTipo = %s", 
			GetSQLValueString($row_recordset["idTipo"], "int")
		);
