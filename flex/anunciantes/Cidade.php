...
function findAll() {
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `cidade` $where"); 
	...
	$query_recordset = "SELECT id,cidade FROM `cidade` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `cidade` (cidade) VALUES (%s)" ,			GetSQLValueString($_REQUEST["cidade"], "text")# 
	);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `cidade` WHERE id = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `cidade` SET cidade = %s WHERE id = %s", 
			GetSQLValueString($_REQUEST["cidade"], "text"), 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `cidade` WHERE id = %s", 
			GetSQLValueString($row_recordset["id"], "int")
		);
