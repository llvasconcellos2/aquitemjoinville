<?php
require_once(dirname(__FILE__) . "/Aquitemjoinvilleconn.php");
...
function findAll() {
	 ...
	 */
	$fields = array('id','nome','observacao','endereco','telefone1','telefone2','telefone3','site','tipo','cidade');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `anunciantes` $where"); 
	...
	$query_recordset = "SELECT id,nome,observacao,endereco,telefone1,telefone2,telefone3,site,tipo,cidade FROM `anunciantes` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `anunciantes` (nome,observacao,endereco,telefone1,telefone2,telefone3,site,tipo,cidade) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["nome"], "text"), # 
			GetSQLValueString($_REQUEST["observacao"], "text"), # 
			GetSQLValueString($_REQUEST["endereco"], "text"), # 
			GetSQLValueString($_REQUEST["telefone1"], "text"), # 
			GetSQLValueString($_REQUEST["telefone2"], "text"), # 
			GetSQLValueString($_REQUEST["telefone3"], "text"), # 
			GetSQLValueString($_REQUEST["site"], "text"), # 
			GetSQLValueString($_REQUEST["tipo"], "int"), # 
			GetSQLValueString($_REQUEST["cidade"], "int")# 
	);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `anunciantes` WHERE id = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `anunciantes` SET nome = %s,observacao = %s,endereco = %s,telefone1 = %s,telefone2 = %s,telefone3 = %s,site = %s,tipo = %s,cidade = %s WHERE id = %s", 
			GetSQLValueString($_REQUEST["nome"], "text"), 
			GetSQLValueString($_REQUEST["observacao"], "text"), 
			GetSQLValueString($_REQUEST["endereco"], "text"), 
			GetSQLValueString($_REQUEST["telefone1"], "text"), 
			GetSQLValueString($_REQUEST["telefone2"], "text"), 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `anunciantes` WHERE id = %s", 
			GetSQLValueString($row_recordset["id"], "int")
		);
